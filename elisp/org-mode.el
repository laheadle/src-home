...
(bind-key "M-c M-c" 'bh/show-org-agenda)

(bind-key "M-c c" 'calendar)
(bind-key "M-c f" 'boxquote-insert-file)

(bind-key "M-c h" 'bh/hide-other)
(bind-key "M-c n" 'bh/toggle-next-task-display)

(bind-key "M-c I" 'bh/punch-in)
(bind-key "M-c O" 'bh/punch-out)

(bind-key "M-c o" 'bh/make-org-scratch)

(bind-key "M-c r" 'boxquote-region)
(bind-key "M-c s" 'bh/switch-to-scratch)

(bind-key "M-c t" 'bh/insert-inactive-timestamp)
(bind-key "M-c T" 'bh/toggle-insert-inactive-timestamp)

(bind-key "M-c v" 'visible-mode)
(bind-key "M-c l" 'org-toggle-link-display)
(bind-key "M-c SPC" 'bh/clock-in-last-task)
(bind-key "C-<" 'previous-buffer)
;(bind-key "M-M-c" 'org-toggle-inline-images)
...
(setq org-agenda-custom-commands
      (quote (("y" agenda*)
              ...
              ("b" "My Todos" tags-todo "-DONE-CANCELLED"
               ((org-agenda-overriding-header "My Todos")
                (org-agenda-skip-function 'my-org-agenda-skip-all-siblings-but-first)))
              ("c" "My Todos 2" tags-todo "-DONE-CANCELLED"
               ((org-agenda-overriding-header "My Todos 2")
               ...
               ((agenda "" nil)
                (tags "REFILE"
                      ((org-agenda-overriding-header "Tasks to Refile")
                       (org-tags-match-list-sublevels nil)))
                ;; To make sure that only lines are checked that
                ;; actually have any TODO keyword (resulting in a
                ;; speed-up), start the TODO part after the slash with
                ;; `!'.  Using `/!' will not match TODO keywords in a
                ;; DONE state.
                (tags-todo "-CANCELLED/!"
...
(defun lh/skip-task-descendents ()
  ""
  (save-restriction
    (widen)
    (let ((next-headline (save-excursion (or (outline-next-heading) (point-max)))))
      (if (lh/has-todo-ancestor)
          next-headline
        nil
))))
